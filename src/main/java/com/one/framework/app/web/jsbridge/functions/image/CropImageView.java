...
public class CropImageView extends ImageView implements View.OnTouchListener,
  ...
  private static final int BORDERDISTANCE = CropView.BORDERDISTANCE;
  ...
  private void configPosition() {
    ...
    minScale = Math
        .max((float) borderWidthLength / drawableHeight, (float) borderWidthLength / drawableWidth);
    /**
  ...
  private class MultiGestureDetector extends GestureDetector.SimpleOnGestureListener implements
    ...
    public boolean onScale(ScaleGestureDetector detector) {
      ...
      if (getDrawable() != null && ((scale < maxScale && scaleFactor > 1.0f) || (scale > minScale
          && scaleFactor < 1.0f))) {
  ...
  private class AnimatedZoomRunnable implements Runnable {
    ...
    public void run() {
      ...
      if (((deltaScale > 1f) && (currentScale < targetZoom)) || ((deltaScale < 1f) && (targetZoom
          < currentScale))) {
  ...
  public Bitmap crop(float outputX, float outputY) {
    ...
    try {
      bitmap = Bitmap.createBitmap(
          d.getBitmap(), (int) (relativeLeft / curScale), (int) (relativeTop / curScale),
          (int) (borderWidthLength / curScale),
