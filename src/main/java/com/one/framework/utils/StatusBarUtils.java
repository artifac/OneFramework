...
public class StatusBarUtils {
  ...
  private static final int FAKE_STATUS_BAR_VIEW_ID = R.id.system_ui_fake_status_bar_view;
  private static final int FAKE_TRANSLUCENT_VIEW_ID = R.id.system_ui_translucent_view;
  ...
  //设置6.0 状态栏深色浅色切换
  private static boolean setCommonStatusBarUI(Activity activity, boolean dark) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
  ...
  public static boolean setStatusBarFontIconDark(Activity activity, @ViewType int type,boolean dark) {
    switch (type) {
      case TYPE_MIUI:
        return setMIUIStatusBarDarkIcon(activity, dark);
      case TYPE_FLYME:
        return setMeizuStatusBarDarkIcon(activity, dark);
      case TYPE_M:
      default:
        return setCommonStatusBarUI(activity,dark);
    }
  }
}
